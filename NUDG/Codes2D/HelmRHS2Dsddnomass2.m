%this is wrong. tried to do central the way I implemented LDG
%must've overlooked something.

function [rhsu,Mu] = HelmRHS2Dsddnomass2(u,roota)
...
% same as HelmRHS2Dsdd but returns a vector instead
...
%modify + traces for BC's
rootaqxP(vmapN) = -rootaqxM(vmapN);
rootaqyP(vmapN) = -rootaqyM(vmapN);
...
rhsu = divrootaq - u - LIFT*(Fscale.*fluxq);
Mu = J.*((invV'*invV)*u);
return;