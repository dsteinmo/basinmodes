...
% OP*u would give MM*Dy*u. Uses central fluxes (no penalty terms, yet)
...
for k1=1:K 
  ...
  OP11 = J(1,k1)*(MassMatrix*Dy); %volume contribution
  ...
  for f1=1:Nfaces
    ...
    switch(BCType(k1,f1))
      case {Dirichlet}    %if we're on a Dirichlet bdry, put in appropriate term
	      OP11 = OP11 - ( mmE*lny  ); % subtracted ghost term is negative of '-' term, so just get twice '-' term
                                      % i.e., other way of writing this is:
                                      %OP11 = OP11 - 0.5*(mmE*lny - (-mmE*lny)) % second term is from ghosting
	...
	%OP11        = OP11 + 0.5*( mmE*lny ); %had this
    OP11        = OP11 - 0.5*( mmE*lny ); %think this is right.
    ...
    %OP12(:,Fm2) = OP12(:,Fm2) - 0.5*mmE(:,Fm1)*lny; %had this
    OP12(:,Fm2) = OP12(:,Fm2) + 0.5*mmE(:,Fm1)*lny; %think this is right
