function [TRI,xout,yout,uout,interp,handle] = PlotField2D_1tri(Nout, xin, yin, uin)
...
handle = trisurf(TRI, xout(:), yout(:), uout(:));
xlim([-1 1]); ylim([-1 1]);
shading interp; axis tight;
%material shiny,    
%lighting gouraud   %comment these 3 and use  view([0 90]) for a pcolor effect
%camlight headlight
return
